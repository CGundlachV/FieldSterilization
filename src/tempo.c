...
static void lv_tick_timer_handler(struct k_timer *timer)
{
	lv_tick_inc(1);
}

K_TIMER_DEFINE(lv_tick_timer,lv_tick_timer_handler, NULL);
...
int main(void)
{
	...
	LOG_INF("Grey Scale Sleep: %dms",grey_scale_sleep);
	...
	fill_buffer_fnc(TOP_LEFT, 200U, buf, buf_size);
	x = 0;
	y = 0;
	display_write(display_dev, x, y, &buf_desc, buf);

	fill_buffer_fnc(TOP_RIGHT, 200U, buf, buf_size);
	x = capabilities.x_resolution - rect_w;
	y = 0;
	display_write(display_dev, x, y, &buf_desc, buf);

	fill_buffer_fnc(BOTTOM_RIGHT, 200U, buf, buf_size);
	x = capabilities.x_resolution - rect_w;
	...
	while (1) {
		x = 0;
		y = 0;
		fill_buffer_fnc(TOP_LEFT, grey_count, buf, buf_size);
		display_write(display_dev, x, y, &buf_desc, buf);
		x = capabilities.x_resolution - rect_w;
		y = 0;
		fill_buffer_fnc(TOP_RIGHT, grey_count, buf, buf_size);
		display_write(display_dev, x, y, &buf_desc, buf);
		x = capabilities.x_resolution - rect_w;
		y = capabilities.y_resolution - rect_h;
		fill_buffer_fnc(BOTTOM_RIGHT, grey_count, buf, buf_size);
		display_write(display_dev, x, y, &buf_desc, buf);
